...
EXTRA_CSS = r"""
  ...
  .zone-path{fill:transparent;stroke:transparent;stroke-width:0.55;stroke-dasharray:1.7 1.15;pointer-events:none;}
  .zone-path.is-hover,.zone-path.is-active{fill:rgba(74,163,212,0.16);stroke:#4aa3d4;}
  .zone-hit{fill:transparent;stroke:transparent;cursor:pointer;}
...
const SHAPES = [
  {key:'forehead', badges:[{x:50,y:26.5}], paths:['M34.5 26.5a15.5 7.2 0 1 0 31 0a15.5 7.2 0 1 0 -31 0']},
  {key:'glabella', badges:[{x:50,y:35.2}], paths:['M44.8 35.2a5.2 3 0 1 0 10.4 0a5.2 3 0 1 0 -10.4 0']},
  {key:'eyes', badges:[{x:36.5,y:39.8},{x:63.5,y:39.8}], paths:['M31.3 39.8a8.2 3.4 0 1 0 16.4 0a8.2 3.4 0 1 0 -16.4 0','M52.3 39.8a8.2 3.4 0 1 0 16.4 0a8.2 3.4 0 1 0 -16.4 0']},
  {key:'cheeks', badges:[{x:38,y:50.5},{x:62,y:50.5}], paths:['M30.2 50.5a7.8 8.8 0 1 0 15.6 0a7.8 8.8 0 1 0 -15.6 0','M54.2 50.5a7.8 8.8 0 1 0 15.6 0a7.8 8.8 0 1 0 -15.6 0']},
  {key:'nasolabial', badges:[{x:44.8,y:53.8},{x:55.2,y:53.8}], paths:['M41.2 53.8a3.6 5.2 0 1 0 7.2 0a3.6 5.2 0 1 0 -7.2 0','M51.6 53.8a3.6 5.2 0 1 0 7.2 0a3.6 5.2 0 1 0 -7.2 0']},
  {key:'lips', badges:[{x:50,y:57.2}], paths:['M41.6 57.2a8.4 3.4 0 1 0 16.8 0a8.4 3.4 0 1 0 -16.8 0']},
  {key:'chin', badges:[{x:50,y:66.4}], paths:['M41.8 66.4a8.2 5 0 1 0 16.4 0a8.2 5 0 1 0 -16.4 0']},
  {key:'jawline', badges:[{x:35.8,y:61.5},{x:64.2,y:61.5}], paths:['M31.2 61.5a4.6 8 0 1 0 9.2 0a4.6 8 0 1 0 -9.2 0','M59.6 61.5a4.6 8 0 1 0 9.2 0a4.6 8 0 1 0 -9.2 0']},
  {key:'neck', badges:[{x:44.8,y:82},{x:55.2,y:82}], paths:['M40.4 82a4.4 11.5 0 1 0 8.8 0a4.4 11.5 0 1 0 -8.8 0','M50.8 82a4.4 11.5 0 1 0 8.8 0a4.4 11.5 0 1 0 -8.8 0']},
  {key:'decollete', badges:[{x:50,y:110}], paths:['M18 110a32 8.5 0 1 0 64 0a32 8.5 0 1 0 -64 0']}
];
...
function renderFace(){
  ...
  SHAPES.forEach(shape=>{
    const on = faceActive===shape.key;
    shape.paths.forEach(d=>{
      markup += `<path class="zone-hit" data-zone="${shape.key}" d="${d}"></path>`;
      markup += `<path class="zone-path${on?' is-active':''}" d="${d}"></path>`;
    });
  });
  SHAPES.forEach(shape=>{
    ...
    (shape.badges||[]).forEach(badge=>{
      markup += `<g data-zone="${shape.key}" style="cursor:pointer">
      <circle cx="${badge.x}" cy="${badge.y}" r="${on?3.2:2.9}" fill="${on?'rgba(74,163,212,0.92)':'rgba(247,249,251,0.85)'}" stroke="rgba(74,163,212,0.7)" stroke-width="0.35"></circle>
      <text x="${badge.x}" y="${badge.y+1.0}" text-anchor="middle" font-size="3" font-weight="600" font-family="Manrope,sans-serif" fill="${on?'#fff':'rgba(61,146,194,0.95)'}">${meta.n}</text>
    </g>`;
    });
  });
